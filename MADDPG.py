...
class MADDPG:
    ...
    def sample(self, batch_size):  # 从buffer中抽样
    ...
    def select_action(self, obs):
        ...
        for agent, o in obs.items():
            ...
            # print("o=",o)
            a = self.agents[agent].action(o).to(self.device)  # torch.Size([1, action_size])

            # print("a=", a)
            # NOTE that the output is a tensor, convert it to int before input to the environment
            # actions[agent] = a.squeeze(0).argmax().item()  # 需要离散取值的函数改为连续取值
            actions[agent] = a  # 需要离散取值的函数改为连续取值
            self.logger.info(f'{agent} action: {actions[agent]}')
            # actions = actions.clamp(0, 1)  # 裁剪到（0，1）
        # actions = {}
        # for agent, o in obs.items():
        #     o = torch.from_numpy(o).unsqueeze(0).float().to(self.device)
        #     a = self.agents[agent].action(o).to(self.device)   # torch.Size([1, action_size])
    ...
    def learn(self, batch_size, gamma):
        for agent_id, agent in self.agents.items():
            obs, act, reward, next_obs, done, next_act = self.sample(batch_size)  # 抽样
            # update critic
